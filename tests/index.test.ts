...
describe('instantiate client', () => {
  ...
  describe('defaultQuery', () => {
    test('with null query params given', () => {
      const client = new Honcho({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo' } });
    ...
    test('overriding with `undefined`', () => {
      const client = new Honcho({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Honcho({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    test('explicit option', () => {
      const client = new Honcho({ baseURL: 'https://example.com' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Honcho({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Honcho({});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Honcho({ timeout: 10, fetch: testFetch });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Honcho({ fetch: testFetch });
