...
describe('resource users', () => {
  test('create: only required params', async () => {
    const responsePromise = client.apps.users.create('app_id', { name: 'x' });
  ...
  test('create: required and optional params', async () => {
    const response = await client.apps.users.create('app_id', { name: 'x', metadata: { foo: 'bar' } });
  });
  ...
  test('get', async () => {
    const responsePromise = client.apps.users.get('app_id');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('get: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.apps.users.get('app_id', { path: '/_stainless_unknown_path' })).rejects.toThrow(
      Honcho.NotFoundError,
    );
  });
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.apps.users.get('app_id', { user_id: 'user_id' }, { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Honcho.NotFoundError);
  });
